...
$nombre = (isset($_POST['inombre'])) ? trim(strtoupper($_POST['inombre'])) : '';
$appat = (isset($_POST['iappat'])) ? trim(strtoupper($_POST['iappat'])) : '';
$apmat = (isset($_POST['iapmat'])) ? trim(strtoupper($_POST['iapmat'])) : '';
$celular = (isset($_POST['icel'])) ? $_POST['icel'] : '';
$direccion = (isset($_POST['idir'])) ? trim(strtoupper($_POST['idir'])) : '';
...
//Parametros para cambio de foto
...
switch($opcion){
    case 1: //GUARDAR USUARIO
        $consulta = "INSERT into persona values (null,'$dni','$appat','$apmat','$nombre','','','','','$direccion','','','$celular')";			
        $resultado = $conexion->prepare($consulta);
        $resultado->execute(); 
        
        $consulta = "INSERT into usuarios values (null,'$usuario','$dni','$contraseña','$email',sysdate(),'',sysdate(),'DESACTIVADO','files/images/0/persona.png','$rol')";			
        $resultado = $conexion->prepare($consulta);
        $resultado->execute(); 
    ...
    case 2: //EDITAR USUARIO  
        ...
        if ($data['total'] == 0) {
                ...
                $consulta = "UPDATE persona SET dni='$dni', ap_paterno='$appat', ap_materno='$apmat', nombres='$nombre', telefono='$celular', direccion='$direccion'
    ...
    case 3: //Eliminacion en cascada de usuarios
        $consulta = "SELECT count(*) total FROM medico where idpersona=(select idpersona from persona where dni='$dni')";		
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();  
        $data=$resultado->fetch(PDO::FETCH_ASSOC);

        if ($data['total'] == 0) {
            $consulta = "SELECT count(*) total FROM paciente where idpersona=(select idpersona from persona where dni='$dni')";		
            $resultado = $conexion->prepare($consulta);
            $resultado->execute();  
            $data=$resultado->fetch(PDO::FETCH_ASSOC);

            if ($data['total'] == 0) {
                $consulta = "SELECT count(*) total FROM pers_adm where idpersona=(select idpersona from persona where dni='$dni')";		
                $resultado = $conexion->prepare($consulta);
                $resultado->execute();  
                $data=$resultado->fetch(PDO::FETCH_ASSOC);
        ...
        break;
    case 4:    
        $consulta = "SELECT idusuarios, nombre, dni, rol, email, estado FROM usuarios u, roles r where r.idroles=u.idroles";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();        
        $data=$resultado->fetchAll(PDO::FETCH_ASSOC);
        break;
    ...
    case 7: //Validacion de existencia de EMAIL
        $consulta = "select count(*) total from usuarios where email='$email'";
        $resultado = $conexion->prepare($consulta);
        $resultado->execute();        
        $data=$resultado->fetch(PDO::FETCH_ASSOC);
    ...
    case 12:
        ...
        if ($data['total'] == 0) {
                ...
                $consulta = "UPDATE persona SET  ap_paterno='$appat', ap_materno='$apmat', nombres='$nombre', telefono='$celular', direccion='$direccion'
                WHERE idpersona='$idper'";		
                $resultado = $conexion->prepare($consulta);
                $resultado->execute(); 
